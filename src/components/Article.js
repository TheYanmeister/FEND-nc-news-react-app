...
const Article = props => {
  const article = articles.filter(
    article => article.article_id === +props.article_id
  )[0];
  const createdAt = article.created_at.replace(/[A-Z]/g, " ").slice(0, -8);
  return (
    <section>
      <h1>{article.title}</h1>
      <h4>
        Posted on: {formatDate(createdAt)} At: {createdAt.slice(11)}
      </h4>
      <p>{article.body}</p>
